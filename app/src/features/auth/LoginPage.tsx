      ...
      <div className="relative w-[380px]">
        <div className="surface-float px-8 pt-9 pb-7">
          ...
          {form ? (
            ...
            <button onClick={() => setForm(true)} className="mt-5 w-full border-t border-[rgba(19,23,34,.07)] pt-4 text-center text-[13px] font-medium text-faint transition hover:text-ink">계정으로 로그인</button>
          )}
        </div>
        <p className="mt-4 text-center text-xs text-dim">합성 데이터 기반 데모. 법적 판단을 대행하지 않습니다</p>
      </div>
    </div>
  )
}
