  ...
  return (
    <Dialog open={open} onOpenChange={v => { if (!v) close() }}>
      <DialogContent className="max-w-[460px] p-6">
        <DialogTitle className="text-[19px] font-semibold tracking-[-0.01em] text-ink">가상 금융회사 와우페이의 콘솔입니다</DialogTitle>
        <ol className="mt-3.5">
          {[
            ...
            <li key={t} className="flex gap-2.5 border-t border-[rgba(19,23,34,.06)] py-2.5 first:border-t-0 first:pt-0">
              ...
              <span className="text-[13.5px] leading-5"><b className="block font-semibold text-ink">{t}</b><span className="block text-body">{d}</span></span>
            </li>
          ))}
        </ol>
